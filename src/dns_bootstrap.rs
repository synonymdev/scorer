...
/// Maximum number of retry attempts for DNS queries
...
///
/// Implements BOLT #10 DNS seed queries using SRV records.
...
impl DnsBootstrapClient {
	...
	/// * `num_peers` - Number of peers to request from the seed
	...
	pub async fn query_bootstrap_peers(&self) -> Result<Vec<BootstrapPeer>, DnsBootstrapError> {
		...
		for attempt in 1..=MAX_RETRIES {
			match self.query_srv_records(&query_domain).await {
				...
				Err(e) => {
					eprintln!(
						"DNS bootstrap attempt {}/{} failed for {}: {:?}",
						attempt, MAX_RETRIES, query_domain, e
					);
					last_error = Some(e);
		...
		Err(last_error.unwrap_or(DnsBootstrapError::RetriesExhausted))
	}

	/// Build DNS query domain per BOLT #10
	///
	/// Format: r<realm>.a<address_types>.n<num_peers>.<seed>
	/// Example: r0.a6.n25.lseed.bitcoinstats.com
	///
	/// Conditions are evaluated right-to-left by the DNS seed.
	fn build_query_domain(&self) -> String {
		format!("r{}.a{}.n{}.{}", self.realm, self.address_types, self.num_peers, self.seed)
	}
...
mod tests {
	...
	fn test_build_query_domain() {
		...
		let domain = client.build_query_domain();
		assert_eq!(domain, "r0.a6.n25.lseed.bitcoinstats.com");
	}
	...
	fn test_build_query_domain_custom_params() {
		...
		let domain = client.build_query_domain();
		assert_eq!(domain, "r1.a2.n10.test.example.com");
	}
